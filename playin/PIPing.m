...
typedef struct PIIPv4Header PIIPv4Header;

__Check_Compile_Time(sizeof(PIIPv4Header) == 20);
__Check_Compile_Time(offsetof(PIIPv4Header, versionAndHeaderLength) == 0);
__Check_Compile_Time(offsetof(PIIPv4Header, differentiatedServices) == 1);
__Check_Compile_Time(offsetof(PIIPv4Header, totalLength) == 2);
__Check_Compile_Time(offsetof(PIIPv4Header, identification) == 4);
__Check_Compile_Time(offsetof(PIIPv4Header, flagsAndFragmentOffset) == 6);
__Check_Compile_Time(offsetof(PIIPv4Header, timeToLive) == 8);
__Check_Compile_Time(offsetof(PIIPv4Header, protocol) == 9);
__Check_Compile_Time(offsetof(PIIPv4Header, headerChecksum) == 10);
__Check_Compile_Time(offsetof(PIIPv4Header, sourceAddress) == 12);
__Check_Compile_Time(offsetof(PIIPv4Header, destinationAddress) == 16);
...
static uint16_t pi_in_cksum(const void *buffer, size_t bufferLen) {
    //
    size_t              bytesLeft;
    int32_t             sum;
...
- (void)didFailWithError:(NSError *)error {
    id<PIPingDelegate>  strongDelegate;
    ...
    if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pi_ping:didFailWithError:)] ) {
        [strongDelegate pi_ping:self didFailWithError:error];
    }
}
...
- (NSData *)pingPacketWithType:(uint8_t)type payload:(NSData *)payload requiresChecksum:(BOOL)requiresChecksum {
    ...
    if (requiresChecksum) {
        ...
        icmpPtr->checksum = pi_in_cksum(packet.bytes, packet.length);
    }
...
- (NSData *)packetWithPingData:(NSData *)data {
    ...
    // data may be nil
    NSParameterAssert(self.hostAddress != nil);     // gotta wait for -PIPing:didStartWithAddress:
    ...
    switch (self.hostAddressFamily) {
        case AF_INET: {
            packet = [self pingPacketWithType:PIICMPv4TypeEchoRequest payload:payload requiresChecksum:YES];
        } break;
        case AF_INET6: {
            packet = [self pingPacketWithType:PIICMPv6TypeEchoRequest payload:payload requiresChecksum:NO];
        } break;
        default: {
            assert(NO);
        } break;
...
- (void)sendPacket:(nonnull NSData *)packet {
    ...
    if ( (bytesSent > 0) && (((NSUInteger) bytesSent) == packet.length) ) {
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pi_ping:didSendPacket:sequenceNumber:)] ) {
            [strongDelegate pi_ping:self didSendPacket:packet sequenceNumber:self.nextSequenceNumber];
        }
    } else {
        NSError *   error;
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pi_ping:didFailToSendPacket:sequenceNumber:error:)] ) {
            [strongDelegate pi_ping:self didFailToSendPacket:packet sequenceNumber:self.nextSequenceNumber error:error];
        }
    }
...
+ (NSUInteger)icmpHeaderOffsetInIPv4Packet:(NSData *)packet timeToLive:(NSInteger *)timeToLive {
    ...
    NSUInteger                  result;
    const struct PIIPv4Header *   ipPtr;
    size_t                      ipHeaderLength;
    
    result = NSNotFound;
    if (packet.length >= (sizeof(PIIPv4Header) + sizeof(PIICMPHeader))) {
        ipPtr = (const PIIPv4Header *) packet.bytes;
        if ( ((ipPtr->versionAndHeaderLength & 0xF0) == 0x40) &&            // IPv4
            ( ipPtr->protocol == IPPROTO_ICMP ) ) {
            ipHeaderLength = (ipPtr->versionAndHeaderLength & 0x0F) * sizeof(uint32_t);
            if (packet.length >= (ipHeaderLength + sizeof(PIICMPHeader))) {
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr timeToLive:(NSInteger *)timeToLive {
    ...
    if (icmpHeaderOffset != NSNotFound) {
        icmpPtr = (struct PIICMPHeader *) (((uint8_t *) packet.mutableBytes) + icmpHeaderOffset);
        receivedChecksum   = icmpPtr->checksum;
        icmpPtr->checksum  = 0;
        calculatedChecksum = pi_in_cksum(icmpPtr, packet.length - icmpHeaderOffset);
        icmpPtr->checksum  = receivedChecksum;
        
        if (receivedChecksum == calculatedChecksum) {
            if ( (icmpPtr->type == PIICMPv4TypeEchoReply) && (icmpPtr->code == 0) ) {
...
- (BOOL)validatePing6ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (packet.length >= sizeof(*icmpPtr)) {
        ...
        if ( (icmpPtr->type == PIICMPv6TypeEchoReply) && (icmpPtr->code == 0) ) {
            if ( OSSwapBigToHostInt16(icmpPtr->identifier) == self.identifier ) {
                uint16_t    sequenceNumber;
...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber timeToLive:&timeToLive] ) {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pi_ping:didReceivePingResponsePacket:timeToLive:sequenceNumber:timeElapsed:)] ) {
                NSTimeInterval timeElapsed = [[NSDate date] timeIntervalSinceDate:self.pingStartDate];
                [strongDelegate pi_ping:self didReceivePingResponsePacket:packet timeToLive:timeToLive sequenceNumber:sequenceNumber timeElapsed:timeElapsed];
            }
        } else {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pi_ping:didReceiveUnexpectedPacket:)] ) {
                [strongDelegate pi_ping:self didReceiveUnexpectedPacket:packet];
            }
        }
    } else {
...
static void STSocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge PIPing *) info;
    assert([obj isKindOfClass:[PIPing class]]);
...
- (void)startWithHostAddress {
    ...
    if (err != 0) {
        ...
        strongDelegate = self.delegate;
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(pi_ping:didStartWithAddress:)] ) {
            [strongDelegate pi_ping:self didStartWithAddress:self.hostAddress];
        }
    }
    assert(fd == -1);
}
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                ...
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        ...
                        if (self.addressStyle != PIPingAddressStyleICMPv6) {
                    ...
                    case AF_INET6: {
                        ...
                        self.IPAddress = [NSString stringWithFormat:@"%s", s];
                        if (self.addressStyle != PIPingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
